...
test("falha de credencial degrada somente Instagram e abre alerta",()=>{assert.match(instagram,/status:"degraded"/);assert.match(instagram,/Instagram Messaging API/);assert.match(instagram,/integration_health_events/);assert.doesNotMatch(instagram,/status:"error"/)});
test("falha de credencial degrada somente Messenger e abre alerta",()=>{assert.match(facebook,/messenger_access_token_invalid/);assert.match(facebook,/Facebook Messenger API/);assert.match(facebook,/integration_health_events/)});
