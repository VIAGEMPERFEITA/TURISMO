...
type OpenAIResponse={output_text?:string;output?:OpenAIOutput[];error?:{code?:string}};
type HistoryMessage={id:string;direction:string;body:string|null};
const outputText=(response:OpenAIResponse)=>clean(response.output_text||(response.output||[]).flatMap(x=>x.content||[]).filter(x=>x.type==="output_text").map(x=>x.text||"").join("\n"),3000);
...
Deno.serve(async request=>{
  ...
  const executionId=clean((await request.json().catch(()=>({})))?.executionId,64);
  if(!executionId)return json({error:"execution_id_required"},400);
  const {data:execution}=await admin.from("social_automation_executions").select("id,organization_id,conversation_id,source_message_id,status,messaging_window_expires_at").eq("id",executionId).single();
  ...
  const {data:conversation}=await admin.from("conversations").select("id,organization_id,lead_id,channel,control_mode,ai_managed,requires_human,assigned_to,status,external_thread_id,channel_account_id").eq("id",execution.conversation_id).single();
  if(!conversation)return json({error:"conversation_not_found"},404);
  if(conversation.channel!=="facebook"||conversation.control_mode!=="ia"||!conversation.ai_managed||conversation.requires_human||conversation.assigned_to||conversation.status==="encerrada"){
  ...
  const {data:source}=await admin.from("messages").select("id,body,message_type,direction").eq("id",execution.source_message_id).single();
  ...
  try{
    ...
    const history=((historyResult.data||[]) as HistoryMessage[]).filter(m=>m.id!==source.id).reverse().map(m=>({role:m.direction==="entrada"?"user":"assistant",content:clean(m.body,1500)}));
    const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${openaiKey}`,"Content-Type":"application/json"},body:JSON.stringify({
      ...
      input:[...history,{role:"user",content:clean(source.body,4000)}],reasoning:{effort:"low"},text:{verbosity:"low"},max_output_tokens:500,safety_identifier:`vp_facebook_${conversation.id.replaceAll("-","").slice(0,32)}`,
    })});
    const provider=await response.json().catch(()=>({})) as OpenAIResponse;
    ...
    const messageInsert=await admin.from("messages").insert({conversation_id:conversation.id,direction:"saida",message_type:"texto",body:answer,provider:"meta_messenger",author_type:"ia",delivery_status:"pendente",metadata:{execution_id:execution.id}}).select("id").single();
    if(messageInsert.error)throw messageInsert.error;
    const outboundInsert=await admin.from("messenger_outbound_messages").insert({organization_id:conversation.organization_id,channel_account_id:conversation.channel_account_id,execution_id:execution.id,conversation_id:conversation.id,message_id:messageInsert.data.id,recipient_id:conversation.external_thread_id,payload:{text:answer},idempotency_key:`ai:${source.id}`}).select("id").single();
    if(outboundInsert.error)throw outboundInsert.error;
    const dispatch=await fetch(`${supabaseUrl}/functions/v1/facebook-dispatch`,{method:"POST",headers:{Authorization:`Bearer ${serviceKey}`,apikey:serviceKey,"x-worker-secret":workerSecret,"Content-Type":"application/json"},body:JSON.stringify({outboundId:outboundInsert.data.id})});
