...
Deno.serve(async request=>{
  ...
  await admin.from("channel_accounts").update({status:"disabled",last_error:"superseded_by_instagram_reconnection",updated_at:connectedAt})
    .eq("organization_id",profile.organization_id).eq("channel","instagram").neq("id",connected.id).eq("status","connected");
