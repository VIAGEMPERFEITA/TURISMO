...
export function WhatsAppContactProvider({children}:{children:ReactNode}){
  ...
  </div>:<div className="request-review"><div className="review-context"><b>Caravana</b><span>{request.tripName||"Atendimento geral"}</span>{request.destination?<><b>Destinos</b><span>{request.destination}</span></>:null}{request.period?<><b>Período</b><span>{request.period}</span></>:null}{request.duration?<><b>Duração</b><span>{request.duration}</span></>:null}{request.status?<><b>Status</b><span>{request.status}</span></>:null}</div>{(Object.keys(reviewLabels) as Array<keyof LeadFormData>).filter(k=>k!=="consent"&&Boolean(values[k])).map(k=><div key={k}><b>{reviewLabels[k]}</b><span>{String(values[k])}</span></div>)}<p><Check/> Revise os dados acima. Campos vazios não serão enviados.</p></div>}
