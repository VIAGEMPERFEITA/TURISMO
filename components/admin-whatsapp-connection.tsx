...
export function AdminWhatsAppConnection() {
  ...
  useEffect(()=>{
    ...
    const finishAttempt=(nextMessage:string)=>{if(connectionTimeout.current)clearTimeout(connectionTimeout.current);connectionTimeout.current=null;setConnecting(false);setMessage(nextMessage)};
    const onMessage=(event:MessageEvent)=>{if(!event.origin.endsWith("facebook.com"))return;let payload=event.data;if(typeof payload==="string"){try{payload=JSON.parse(payload)}catch{return}}if(payload?.type!=="WA_EMBEDDED_SIGNUP")return;if(payload.event==="FINISH")signupSession.current=payload.data??null;if(payload.event==="CANCEL")finishAttempt("Conexão cancelada. O WhatsApp do celular não foi alterado.");if(payload.event==="ERROR")finishAttempt("A Meta não concluiu a conexão. Nenhuma conta foi removida.")};
    ...
    return()=>{window.removeEventListener("message",onMessage);if(connectionTimeout.current)clearTimeout(connectionTimeout.current)};
  },[]);
  function connect(){if(!window.FB||!sdkReady||connecting)return;setConnecting(true);setMessage("A janela segura da Meta será aberta. Conclua a autorização para continuar.");signupSession.current=null;connectionTimeout.current=setTimeout(()=>{setConnecting(false);setMessage("A autorização da Meta não foi concluída. Tente novamente e permita a abertura da janela de login. O WhatsApp do celular não foi alterado.");connectionTimeout.current=null},120000);window.FB.login(async response=>{if(connectionTimeout.current)clearTimeout(connectionTimeout.current);connectionTimeout.current=null;const code=response.authResponse?.code;if(!code){setConnecting(false);setMessage("A autorização não foi concluída. Nenhuma alteração foi feita no celular.");return}for(let attempt=0;attempt<20&&!signupSession.current;attempt++)await new Promise(resolve=>setTimeout(resolve,250));const session=signupSession.current;if(!session?.waba_id||!session?.phone_number_id){setConnecting(false);setMessage("A Meta não retornou os dados da conta. Confirme que escolheu conectar o WhatsApp Business existente.");return}const client=getSupabaseBrowserClient();if(!client){setConnecting(false);setMessage("Supabase não configurado.");return}const{data,error}=await client.functions.invoke("whatsapp-embedded-signup",{body:{code,...session}});setConnecting(false);if(error||!data?.connected){setMessage(data?.error?`Conexão não concluída: ${data.error}`:"Não foi possível concluir a conexão.");return}setConnected(true);setMessage("WhatsApp Business conectado em coexistência. O aplicativo do celular permanece ativo.")},{config_id:META_CONFIGURATION_ID,response_type:"code",override_default_response_type:true,extras:{setup:{},sessionInfoVersion:"3",featureType:"whatsapp_business_app_onboarding"}})}
