...
export default async function TripPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><PublicHeader/><main className="detail">
    <section className="detail-hero">
      ...
      <div className="shell detail-title"><p>{trip.countries.join(" · ")}</p><h1>{trip.name}</h1><p className="detail-tagline">{trip.shortDescription}</p><div><span><CalendarDays />{period}</span>{trip.days?<span><Clock3 />{trip.days} dias</span>:null}{trip.departureCity?<span><Plane />Saída de {trip.departureCity}</span>:null}{trip.leader ? <span><UserRound />{trip.leader}</span> : null}</div><div className="detail-hero-actions"><WhatsAppLink {...contact} buttonText="Reservar minha vaga" initialInterest="Reservar vaga">Reservar minha vaga</WhatsAppLink><WhatsAppLink {...contact} buttonText="Falar com um consultor" initialInterest="Consultar valores">Falar com um consultor</WhatsAppLink></div></div>
    </section>
    ...
    <div className="shell detail-layout">
      ...
      <aside className="conversion-card"><span>{publicStatusLabels[trip.publicStatus]}</span><h2>{trip.name}</h2><dl><div><dt>Período</dt><dd>{period}</dd></div>{trip.days?<div><dt>Duração</dt><dd>{trip.days} dias</dd></div>:null}{trip.departureCity?<div><dt>Embarque</dt><dd>{trip.departureCity}</dd></div>:null}</dl><strong>{formatTripPrice(trip)}</strong><WhatsAppLink {...contact} buttonText="Reservar minha vaga" initialInterest="Reservar vaga">Reservar minha vaga</WhatsAppLink><WhatsAppLink {...contact} buttonText="Solicitar roteiro" initialInterest="Receber roteiro">Solicitar roteiro</WhatsAppLink><WhatsAppLink {...contact} buttonText="Consultar valores" initialInterest="Consultar valores">Consultar valores</WhatsAppLink></aside>
    </div>
    ...
    <section className="section itinerary shell"><div><p className="eyebrow">Roteiro da viagem</p><h2>Cada dia,<br />uma nova descoberta.</h2><p>Veja abaixo somente as etapas cadastradas para esta caravana.</p></div>{trip.itinerary.length?<ol>{trip.itinerary.map((day) => <li key={day.day}><span>{String(day.day).padStart(2, "0")}</span><div><small>{day.city ? `Dia ${day.day} · ${day.city}` : `Dia ${day.day}`}</small><h3>{day.title}</h3>{day.description ? <p>{day.description}</p> : null}{day.hotel ? <p><b>Hospedagem:</b> {day.hotel}</p> : null}{day.meals?.length ? <p><b>Refeições:</b> {day.meals.join(", ")}</p> : null}{day.activities?.length ? <p><b>Atividades:</b> {day.activities.join(", ")}</p> : null}{day.transportation ? <p><b>Deslocamento:</b> {day.transportation}</p> : null}{day.notes ? <p><b>Observações:</b> {day.notes}</p> : null}</div></li>)}</ol>:<div className="itinerary-request"><FileText/><h3>Solicite o roteiro detalhado.</h3><p>A equipe apresentará a versão oficial disponível para esta saída.</p><WhatsAppLink {...contact} buttonText="Solicitar roteiro" initialInterest="Receber roteiro">Solicitar roteiro</WhatsAppLink></div>}</section>

    <section className="included"><div className="shell"><p className="eyebrow light">Informações da caravana</p>{trip.included.length?<><h2>O pacote inclui</h2><div className="included-grid">{trip.included.map((item) => <span key={item}><Check />{item}</span>)}</div></>:null}{trip.notIncluded.length?<><h3 className="not-included-title">O pacote não inclui</h3><div className="not-included-grid">{trip.notIncluded.map((item) => <span key={item}><Minus />{item}</span>)}</div></>:null}{trip.documentation.length ? <><h3 className="not-included-title">Documentação</h3><div className="not-included-grid">{trip.documentation.map((item) => <span key={item}><FileText />{item}</span>)}</div></> : null}</div></section>
    ...
    <div className="mobile-conversion-bar"><WhatsAppLink {...contact} buttonText="Consultar valores" initialInterest="Consultar valores">Consultar valores</WhatsAppLink><WhatsAppLink {...contact} buttonText="Reservar" initialInterest="Reservar vaga">Reservar</WhatsAppLink></div>
  </main><PublicFooter/></>;
}
