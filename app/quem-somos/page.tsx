...
import { CheckCircle2, Compass, HeartHandshake, ShieldCheck, Users } from "lucide-react";
...
export default function Page(){
  ...
  return <BasicContentPage eyebrow="Quem somos" title="Presença, clareza e propósito em cada jornada." description="A Viagem Perfeita Turismo organiza experiências culturais e espirituais com atendimento próximo, preparação responsável e cuidado humano.">
    <section className="institutional-intro"><div><p className="eyebrow">Porque existimos</p><h2>Para transformar deslocamentos em experiências com significado.</h2><p>Uma viagem de fé começa na escuta. Entendemos o perfil de cada viajante e de cada grupo para construir uma jornada coerente, acolhedora e transparente — desde a primeira conversa até o retorno.</p></div><img src="/media/israel/jerusalem-experiencia.jpg" alt="Registro real de uma experiência da Viagem Perfeita em Jerusalém"/></section>
    ...
    <section className="credibility-card"><div><p className="eyebrow light">Credibilidade</p><h2>Informações institucionais verificáveis.</h2><p>Dados ainda não fornecidos não são inventados nem exibidos ao público. Eles ficam preparados para atualização administrativa.</p></div><dl>{credentials.map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}<div><dt>Instagram</dt><dd><a href={siteConfig.instagram} target="_blank" rel="noopener noreferrer">{siteConfig.instagramHandle}</a></dd></div><div><dt>WhatsApp</dt><dd>(31) 99954-7699</dd></div></dl></section>
    <p className="content-notice"><Users/> Para completar a linha do tempo e apresentar quem coordena as caravanas, ainda são necessárias a história oficial da empresa, nomes autorizados, funções, datas e fotos reais da equipe.</p>
