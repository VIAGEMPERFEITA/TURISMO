...
export async function savePublicLead(data:LeadFormData,context:TripContactContext,buttonText:string):Promise<LeadSaveResult>{
  ...
  const payload={...data,phone_normalized:normalizePhone(data.phone),experience_name:context.tripName||"Atendimento geral",destination:context.destination||null,desired_period:data.desiredPeriod||context.period||null,landing_page:window.location.href,referrer:document.referrer||null,source:"Site",source_detail:buttonText,utm_source:params.get("utm_source"),utm_medium:params.get("utm_medium"),utm_campaign:params.get("utm_campaign"),utm_content:params.get("utm_content"),utm_term:params.get("utm_term"),consent_at:new Date().toISOString()};
